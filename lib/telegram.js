...
export async function sendTelegramNotification(type, payload) {
  ...
  if (type === 'TASK_CREATED') {
    ...
    text = `<b>🔍 Задача №${numDisplay} отправлена на рассмотрение</b>\n\n` +
           `👤 <b>Сдал:</b> ${authorName || 'Пользователь'}`;

    managerDmText = `<b>🔍 Задача №${numDisplay} на рассмотрении</b>\n\n` +
           `📝 <b>Название:</b> ${task?.title || '—'}\n` +
           `📄 <b>Описание:</b> ${task?.description || '—'}\n\n` +
           `👤 <b>Сдал:</b> ${authorName || 'Пользователь'}\n` +
           `⏳ Ожидает приёмки и оценки качества.`;
